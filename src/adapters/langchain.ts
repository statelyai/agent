...
>({ model }: LangChainAgentSettings, promptTemplate: TPrompt) {
    ...
    if (parentSnapshot || !isMachineSnapshot(parentSnapshot)) {
      ...
      (async () => {
        ...
        const stream = await agentExecutor.stream({
          input:
            'use the tools to accomplish the user request, you can use the multiple tools, dont stop until task accomplish',
          ...input,
        });
...
export function fromCallbackChain<
  ...
  TPrompt extends ChatPromptTemplate<TEvent & TInput> = ChatPromptTemplate<
    TEvent & TInput
  >
>(
  ...
  } satisfies InvokeCallback<TEvent, TSentEvent, TInput>);
}

function fromObservableChain<TInput, TOutput>(
  runnable: RunnableSequence<TInput, TOutput>
): ObservableActorLogic<TOutput, TInput> {
  return fromObservable(function ({
    input,
    system,
    self,
  }): Subscribable<TOutput> {
    const observers = new Set<Observer<any>>();
...
function toTools<TSentEvent extends AnyEventObject = AnyEventObject>(
  ...
  function transitionDetails({
    eventType,
    description,
  }: TransitionDefinition<any, TSentEvent>) {
    const schema = events[eventType] as Schema;
    return {
      name: eventType.replace(/\./g, '_'),
      description:
        description ??
        schema?.description ??
        `use this to send an event with the type ${eventType}`,
      eventType: eventType,
      schema: schema,
    };
  }
  ...
  function mapSchemaToZod(value: Schema): z.ZodSchema {
    ...
    if (type === 'array') {
      return z.array(mapSchemaToZod(items as Schema));
    }
    if (type === 'object') {
      return mapObjectSchemaToZod(value as Schema);
    } else {
      return z.unknown();
    }
    function mapObjectSchemaToZod(value: Schema) {
...
export function createLangchainAdapter<T extends LangChainAgentSettings>(
  settings: T
) {
  settings.model;
  return {
