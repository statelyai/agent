...
- [`twenty-questions/index.ts`](twenty-questions/index.ts): decision loop with guard-enforced legality (the last turn must be a guess), where every player turn is an idle state offering Yes/No buttons or free text a classifier interprets — including side questions that cost no turn
...
- [`game-agent/index.ts`](game-agent/index.ts): games as machines, two lessons in one file — a turn-based combat machine whose decision `allowedEvents` are computed from context (HEAL only when low on HP) and whose output is a narrated blow-by-blow, and an interactive rock-paper-scissors match (`rpsMachine`, first to 3) where you throw via `meta.interaction` buttons on an idle state and the model counters using the round history reduced into `context.history`
- [`game-loop-agent/index.ts`](game-loop-agent/index.ts): a long-lived player agent invoked once at the game machine's `playing` state, so it spans substates and rounds — pushed `OBSERVE` events for every move, turn-gated `agent.decide` moves on `YOUR_TURN`, and natural-language round control classified with structured output
- [`go-fish/index.ts`](go-fish/index.ts): hidden-information card game where the model asks and the machine enforces the rules; the human's turn is an idle state resumed by a free-text `ASK` event, with the hand surfaced in the interaction label
...
- [`river-crossing/index.ts`](river-crossing/index.ts): the machine as a verifiable environment: the model proposes wolf/goat/cabbage crossings, guards reject illegal ones (`rejected-by-guard` + retry), a prototype `describeMachine(...)` renders the rules into the decide context, and the run outputs a move-by-move narration of both banks
- [`todo-nl/index.ts`](todo-nl/index.ts): free-text commands mapped onto a real app's machine events (add/toggle/delete) via an explicit `agent.decide` loop, pausing on an idle `awaitingCommand` state (`textEvent: COMMAND`) for the next thing you say; nonexistent ids are guard-rejected mid-loop with a retry, and [`todo-nl/imperative.ts`](todo-nl/imperative.ts) is the same app without the library (raw `generateObject` while-loop) as a deliberate A/B pair
- [`context-compaction/index.ts`](context-compaction/index.ts): a self-managing chat loop: an explicit `compacting` state folds stale messages into a running summary once history exceeds a threshold, with human turns as idle `USER_MESSAGE` events so any host can drive the conversation
