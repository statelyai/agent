/**
 * Eve host — the LLM agent that converses with the user and drives a
 * @statelyai/agent state machine through two tools.
 *
 ...
 *   tools/start_workflow.ts  — bridge: begin the machine, return a handle
 *   tools/resume_workflow.ts — bridge: resume the handle with a human decision
 *
 * Eve auto-discovers `tools/*.ts` by filename, so agent.ts only configures the
 * model. See ./demo.ts for the tool bridge exercised without a live Eve runtime.
 *
