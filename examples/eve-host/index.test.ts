...
import { resumeDraft, startDraft } from "./bridge.js";
import { main } from "./index.js";
import startWorkflow from "./tools/start_workflow.js";
import resumeWorkflow from "./tools/resume_workflow.js";

// The `ctx` Eve's tool loop passes to `execute`; the tools ignore all of it.
const ctx = {
  session: { id: "test" },
  callId: "test",
  toolName: "test",
...
const start = async (prompt: string) => startWorkflow.execute({ prompt }, ctx);

const resume = async (handle: string, eventType: string, text: string | null = null) =>
  resumeWorkflow.execute({ handle, eventType, text }, ctx);
