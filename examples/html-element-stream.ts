...
import {createAgent, createSchemas} from "../src";
...
const elementProperties= {
    ...
    class: {
        type: 'string',
        description: 'Class names of the element.'
    },
    content: {
        type: 'string',
    ...
    data: {
        type: 'array',
        description: 'the data attributes. attributes that start with the prefix data-',
        items: {
            type: 'object',
            properties: {
                name: {
...
export const elementEventSchema = {
    'element.link': {
        type: "object",
        description: 'A link was found on the page.',
        properties: {
            ...elementProperties,
            text: {
                type: 'string',
    ...
    "element.button": {
        type: "object",
        description: 'A button was found on the page.',
    ...
    "element.form": {
        type: "object",
        description: 'A form was found on the page.',
        properties: {
            ...elementProperties,
            action: {
                type: 'string',
    ...
    "element.input": {
        type: "object",
        description: 'An input was found on the page.',
        properties: { 
            ...elementProperties,
            ...inputProperties,
        }
    ...
    "element.input.select": {
        ...
        properties: {
            ...
            options: {
                type: 'array',
                items: {type: 'string'},
                description: 'The available options of the select input.'
            },
            selected: {
                type: 'string',
    ...
    "element.input.checkbox": {
        ...
        properties: {
            ...
            checked: {
                type: 'boolean',
                description: 'The checked status of the checkbox.'
            }
        }
    },
    "element.input.radio": {
        ...
        properties: {
            ...
            selected: {
                type: 'string',
                description: 'The selected option of the radio input.'
            },
            options: {
                type: 'array',
                items: {type: 'string'},
                description: 'The options of the radio input.'
            }
        }
    }
} as const;

function convertOnOf(schema:{name: string, description?: string | undefined, properties: {[p: string]: JSONSchema7}}[]):JSONSchema7 {
    ...
    return {
        properties: schema
            ...
            }), {} as { [p: string]: JSONSchema7 })
    }
}

export const elementArraySchema= convertOnOf(Object.entries(elementEventSchema) .map(([name, schema]) => ({
    name,
    ...schema
}))) 
...
export const elementAgent = createSchemas({ 
    ...
    context: {
        ...
        properties: {
            limit: {
                type: "number",
                description: "The maximum number of elements to extract.",
                default: 100
            },
            html: {
                type: 'string',
            ...
            instructions: {
                type: 'string',
                description: 'The instructions to follow when extracting the elements.'
            },
            elements: elementArraySchema
        }
    }
})
...
const elementStream = llmSettings.fromEvent(   ChatPromptTemplate.fromMessages([
...
const search = llmSettings.fromCallback( 
    ChatPromptTemplate.fromMessages([ SystemMessagePromptTemplate.fromTemplate(`You are a programmer and your job is to pick out information in code to a pm. You are working on an html file. You will extract the necessary content asked from the information provided.
 make sure to include include all data attributes on the elements
...
export const elementMachine = setup({
    types: {
        events: elementAgent.types.events,
        context: {} as  typeof  elementAgent.types.context &{
            elements: HtmlElementMap 
        },
        input: {} as Partial<typeof elementAgent.types.context> & Pick<typeof elementAgent.types.context , "html"> ,
    ...
    actors: {
         'element-stream': elementStream 
             ...
             on: { 
                 ...Object.entries(elementEventSchema).reduce((acc, [name, schema]) => {
                     acc[name] = { 
                         actions: assign({ 
                     ...
                     return acc;}, {} as Record<string, ElementMachineTransition>)
             } 
})
    ...
    inspect: {
        next:(e)=> {
            if(e.type === "@xstate.snapshot"){
                fs.writeFile('html-elements.json', JSON.stringify(actor.getPersistedSnapshot(), null, 2) + '\n',(err)=>{
                    if(err) console.error(err); 
                })
            }
            fs.appendFile('html-elements.log.jsonl', JSON.stringify(e, null, 2), (err)=>{
            if(err) console.error(err);
...
export type ElementMachineContext = ContextFrom<ElementMachine>;
...
export type ElementMachineActor = Actor<ElementMachine>;
 export type ElementMachineSnapshot = ReturnType<ElementMachineActor["getSnapshot"]>;
export type ElementMachineEventObject = ElementMachineEvents[keyof ElementMachineEvents];
export type ElementMachineTransition =  TransitionConfig<ElementMachineContext,ElementMachineEvents, ElementMachineEvents, any, any, any, any>;
