...
**`createScriptedExecutors`: run any agent machine with no API key.**

A keyless, dependency-free executor set (root export) that plays back a script instead of calling a model, so `runAgent`/`provideExecutors` work end to end with nothing installed but core. The quickstart's first run is now keyless.
...
const result = await runAgent(moderationMachine, {
  ...
  executors: createScriptedExecutors({
    // Plain values play back FIFO. For custom logic, an entry can be a
    // function of the request, which says where it was called from:
    // `request.id` is the invoke id, `request.events` the current
    // state's legal candidates, `request.name` the text request's name.
...
- An entry may be the raw executor envelope (`{ output, usage }` / `{ event, reason, usage }`), so scripted runs exercise usage aggregation too. A text entry counts as the envelope only when its own keys are `output` plus optionally `usage`/`raw`; an object with any other key of its own (`{ output: 'draft', confidence: 0.9 }`) is the output value, siblings intact.
