...
- **Inheritance is the default** for any request reached through string-keyed actor sources (invoke `src` strings, registered `actors`), arbitrarily deep, cycle-safe.
- **Explicit bindings win.** A request that carries its own executor (`.withExecutor(...)`, `bindRequestExecutor(...)`, or a child's own `.provide({ actors })`) keeps it. The parent's executors are never called for it.
