...
const run = await runSeam(emailDrafter, {
  ...
  candidate: createAiSdkExecutors({ models }).generateText, // omit to keep the seam scripted
  respond: ({ state }) =>
...
- `candidate` is just an executor, so a live model, a candidate prompt or a fine-tune all plug in; without one the whole run stays scripted.
