...
| Time travel / `get_state_history` | Snapshot list, or `replay` / `fork` | Rewind by replaying a prefix of the log (or restoring an earlier snapshot); fork by appending different events to a new thread. See [`time-travel`](../examples/time-travel/index.ts). |
...
**A conditional edge becomes a guarded transition.** The router is a decision the model makes over named events; the branch is the event's transition, and a guard returning `undefined` makes that branch unavailable:
...
**An interrupt becomes a state that waits.** No checkpointer is configured; the run settles and gives you a snapshot:
