...
Package subpaths are `@statelyai/agent` (root), `@statelyai/agent/ai-sdk`, `@statelyai/agent/sqlite`, and `@statelyai/agent/agent-workflow.json`. Everything a hand-written host needs (`buildEnvelopeSchema`, `getJsonSchema`, `parseOutput`, `parseStructuredEnvelope`, `getAgentOutputMode`, `resolveDecision`, `renderDecisionAttempts`) comes from the root.
