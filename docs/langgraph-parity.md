...
<!-- parity matrix derived from the consolidated pattern examples and docs/langgraph-parity.md scope -->
...
| Branching / conditional routing | [`examples/supervisor/index.test.ts`](/Users/davidkpiano/Code/agent/examples/supervisor/index.test.ts) | Guarded transitions / function-form `on:` handlers (the dedicated conditional-routing port was consolidated into supervisor routing) |
| Subgraphs / nested flows | [`examples/subflows/index.test.ts`](/Users/davidkpiano/Code/agent/examples/subflows/index.test.ts) | A child machine invoked as a nested actor; the parent binds only its own request sources via `runAgent`, the child's sources are bound separately with `.provide(...)` |
| Human-in-the-loop / approval gate | [`examples/human-in-the-loop/index.test.ts`](/Users/davidkpiano/Code/agent/examples/human-in-the-loop/index.test.ts) | Idle-first: a no-invoke state settles `runAgent` to `{ status: 'idle' }`; resume with `{ snapshot, event }` |
| Session restore from snapshots | [`examples/human-in-the-loop/index.test.ts`](/Users/davidkpiano/Code/agent/examples/human-in-the-loop/index.test.ts), [`examples/file-snapshot-store/index.test.ts`](/Users/davidkpiano/Code/agent/examples/file-snapshot-store/index.test.ts) | JSON-serializable `snapshot` round-tripped through a store between `runAgent` calls (the dedicated snapshot-persistence port was consolidated into human-in-the-loop) |
| Streaming side channels | [`examples/sse-transport/index.test.ts`](/Users/davidkpiano/Code/agent/examples/sse-transport/index.test.ts), [`examples/parallel-streams/index.test.ts`](/Users/davidkpiano/Code/agent/examples/parallel-streams/index.test.ts), [`examples/ai-sdk-host/index.ts`](/Users/davidkpiano/Code/agent/examples/ai-sdk-host/index.ts) | `runAgent`'s `onChunk(chunk, { request })`; the machine transitions only on final text |
| Tool calling with intermediate progress | [`examples/tool-calling/index.test.ts`](/Users/davidkpiano/Code/agent/examples/tool-calling/index.test.ts) | Tool calls as host-executed actors reporting through `onChunk`/`onResult` |
| Plan-and-execute | [`examples/plan-and-execute/index.test.ts`](/Users/davidkpiano/Code/agent/examples/plan-and-execute/index.test.ts) | Planner request produces structured output; execution states iterate the plan |
| Map-reduce / fan-out workflows | [`examples/ai-sdk-orchestrator-worker/index.test.ts`](/Users/davidkpiano/Code/agent/examples/ai-sdk-orchestrator-worker/index.test.ts), [`examples/ai-sdk-parallel-review/index.test.ts`](/Users/davidkpiano/Code/agent/examples/ai-sdk-parallel-review/index.test.ts) | Plain `Promise.all(...)` over host actors inside a single invoke — no dedicated fan-out primitive (see alpha status in the readme); the dedicated map-reduce port was consolidated into the AI SDK fan-out examples |
| Reflection / retry loops | [`examples/ai-sdk-evaluator-optimizer/index.test.ts`](/Users/davidkpiano/Code/agent/examples/ai-sdk-evaluator-optimizer/index.test.ts) | Explicit draft/critique/check states sharing a critique schema, re-entered via guarded transition (the dedicated reflection-loop port was consolidated into the evaluator-optimizer example) |
| ReWOO-style planner / worker decomposition | [`examples/plan-and-execute/index.test.ts`](/Users/davidkpiano/Code/agent/examples/plan-and-execute/index.test.ts) | Planner output schema, worker evidence map, and a final solver state (the ReWOO evidence-map idea is kept in plan-and-execute) |
| Supervisor routing | [`examples/supervisor/index.test.ts`](/Users/davidkpiano/Code/agent/examples/supervisor/index.test.ts) | A routing request's structured output selects the target state/actor |
| Multi-agent handoffs | [`examples/swarm-handoff/index.test.ts`](/Users/davidkpiano/Code/agent/examples/swarm-handoff/index.test.ts) | Supervisor routing to typed child actors, persisted across turns via snapshot |
| SQL/tool-heavy agent workflow | [`examples/sql-agent/index.test.ts`](/Users/davidkpiano/Code/agent/examples/sql-agent/index.test.ts) | Query generation, DB execution, and answer synthesis as separate typed states |
...
| Retrieval-augmented generation | [`examples/rag/index.test.ts`](/Users/davidkpiano/Code/agent/examples/rag/index.test.ts) | Retrieval as a typed host actor; generation as a named request |
