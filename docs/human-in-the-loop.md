...
The handler resolves to a `string` (what the human typed), so `onDone`'s `output` needs no cast. For structured input, gather the string(s) and parse/classify in a follow-up state (see [twenty-questions](../examples/twenty-questions/index.ts)) or register a custom actor source in place of `agent.userInput`.
