---
title: The step path
description: Drive an agent machine one external input at a time over an append-only journal, so a durable host can crash and resume by replay.
---
...
Taught from [examples/ai-sdk-game-host/index.ts](../examples/ai-sdk-game-host/index.ts), the canonical thin loop. One host-owned primitive resolves a single frontier effect into the event to journal (or `undefined` for a fire-and-forget action):
...
**Compaction.** A snapshot taken mid-flight cannot carry in-flight effect state; the journal can. So snapshot only at **quiescent / idle** points. Resume from an idle snapshot plus the entries appended since, or replay the whole journal from index 0. Both yield the identical state. See [Checkpoints and the event log](checkpoints.md) for the `AgentEventLogStore` protocol and the snapshot-as-compaction cache.
...
- [Checkpoints and the event log](checkpoints.md): the `AgentEventLogStore` protocol, forking, and time travel.
- [Human in the loop](human-in-the-loop.md): idle states and resuming from a persisted journal.
